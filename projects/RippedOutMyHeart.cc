...
bool GroupOfNumbers::linear_select(long k, long & kth_smallest) const{
  ...
  break;
         ...
         displayGroupOfNumbers();
       cout << "i is: " << i << endl;
      if(_group[j] == _group[left]){
        cout<< j << "j "<<_group[j] << " Well shit" << _group[left]<< endl;
      }
        swap( _group[i], _group[j]);
      if( k < i)
       linearSelect( left, i, k);
      else if( k >= i)
       linearSelect( i, right, k);
      }
      else{
         cout << "inside insert left is " << left << " right is " << right << "Group k " << _group[k - 1];
        insertion_sort(left, right);
      }
    }

  long GroupOfNumbers::median_of_5(long left, long right, long k){
     cout << "inside median left is " << left << " right is " << right;
     displayGroupOfNumbers();
    for(long i = left; i + 4 <= right; i = i + 5){
      quicksort(i, i+4);
      // if((i-left)/5 >= left && (i-left)/5 <= right && i + 2 <= right && i + 2 >= left && left >= 0 && left < right && right < _total)
       // displayGroupOfNumbers();
   ...
   // if(tot + left <)
    ...
    swap(_group[left], _group[tot + left]);
     displayGroupOfNumbers();
     cout << left << "ohe sfd " << right << "Group k " << _group[k] << endl;
    // }
    return _group[left];
  }
